...
import { categoryCreateSchema } from '$lib/schemas/item';
...
export const GET: RequestHandler = async ({ locals }) => {
	...
	try {
		const data = await itemsApi(getServerApi(locals.accessToken)).listCategories();
		return json(data);
	} catch (e) {
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	try {
		const created = await itemsApi(getServerApi(locals.accessToken)).createCategory(parsed.data);
